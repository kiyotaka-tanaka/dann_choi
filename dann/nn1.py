...
class NN():
    ...
    def train_with_valid(self, dataset, testset,learning_rate, batch_size, label_size, test_size, dir_name, model_name, epoch_size, optimizer='AdamOptimizer'):
        ...
        c = 0
        d 
        for i in range(len(argmz)):
            if argmz[i] == 0 and np.argmax(test_lab[i]) == 0:
                a = a +1
            elif argmz[i] == 0 and np.argmax(test_lab[i]) == 1:
                b = b+1
            elif argmz[i] == 1 and np.argmax(test_lab[i]) == 0:
                c = c+1

            else:
                continue

        pr = float(a)/float(a+b)
        re = float(a)/float(a+c)

        f = 2*float(pr)*float(re)/float(pr+re)

        #print "precision is %f recall is %f f is %f" %(pr , re , f)
        ...
        '''
        if  len(argmz) == len(self.test_lab):
            print "yes yes yes yes yes yes yes yes yes yes yes yes "
         '''

        for i in range(len(argmz)):
            if (argmz[i]) == 0 and np.argmax(self.test_lab[i]) == 0:
                a = a +1
            elif (argmz[i]) == 1 and np.argmax(self.test_lab[i]) == 0:
                c = c+1
            elif (argmz[i]) == 0 and np.argmax(self.test_lab[i]) == 1:
                b = b+1
            elif (argmz[i]) == 1 and np.argmax(self.test_lab[i]) == 1:
                d = d + 1
            else:
                continue
        '''
        ...
        for i in range(length):
            if np.argmax(argmz[i]) == np.argmax(test_lab[i]):
                acc = acc + 1
        ...
        print ("a is %d b is %d c is %d d is %d ") %(a,b,c,d)
        return pr, re, f,acc,a,b,c,d
    def define_loss(self):
        ...
        if self.debug_flag == 'sqrt':
            self.loss = tf.sqrt(tf.reduce_mean(tf.square(tf.sub(self.y, y))))
        else:
